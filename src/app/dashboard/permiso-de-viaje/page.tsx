...
export default function PermisoViajeDashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        //("Todos los trámites recuperados:", allTramites);

        const permisoViajeDetails = await ApiDataFetcher.fetchTramitesByType(
          "permiso-de-viaje"
        );
        //console.log("Detalles de permiso-de-viaje:", permisoViajeDetails);

        const matchedTramites = await Promise.all(
          permisoViajeDetails.map(async (detalle: any) => {
        ...
        console.error("Error fetching Permiso de Viaje data:", error);
        setLoading(false);
      }
    }
  ...
  const renderTramitesPorSeccion = (estado: Estado, seccion: string) => {
    const filteredTramites = tramites.filter((tramite) => {
      ...
      if (seccion === "Enviar correo/Lost") {
        return (
          ...
          (!tramite.datoPadres?.[0]?.idADocumentoIdentidadAnverso || !tramite.datoMenores?.[0]?.idADocumentoIdentidadAnverso
          || !tramite.datoPadres?.[0]?.idDocumentoIdentidadReverso || !tramite.datoMenores?.[0]?.idDocumentoIdentidadReverso
        )
        );
      }
    ...
    return filteredTramites.map((tramite) => (
      ...
      >
        <p>
          <strong>Tipo de Permiso:</strong> {tramite.tipoPermisoViaje || "Desconocido"}
        </p>
        <p>
          <strong>Tipo de Documento:</strong> {tramite.datoPadres?.[0]?.tipoDocumento || "Desconocido"}
        </p>
        <p>
          <strong>Número de Documento:</strong> {tramite.datoPadres?.[0]?.numeroDocumento || "No disponible"}
        </p>
        <p>
          <strong>Padre/Madre/Apoderado:</strong> {capitalize(tramite.name + ' ' + tramite.apellidos) || "Desconocido"}
        </p>
        <p>
        ...
        {seccion === "Enviar correo/Lost" && (
          <div>
            <p className="text-red-400">
              ...
              {!tramite.datoPadres?.[0]?.idADocumentoIdentidadAnverso || !tramite.datoMenores?.[0]?.idADocumentoIdentidadAnverso && "Documento Identidad Anverso de los padres, "}
              {!tramite.datoPadres?.[0]?.idDocumentoIdentidadReverso  || !tramite.datoMenores?.[0]?.idDocumentoIdentidadReverso && "Documento Identidad Reverso"}
            </p>
          </div>
        )}
      </div>
