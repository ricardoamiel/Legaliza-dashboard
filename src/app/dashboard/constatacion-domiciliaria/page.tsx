...
export default function ConstatacionDomiciliariaDashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const constatacionDomiciliariaDetails = await ApiDataFetcher.fetchTramitesByType(
          "constatacion-domiciliaria"
        );
        //console.log("Detalles de constatacion-domiciliaria:", constatacionDomiciliariaDetails);

        const matchedTramites = await Promise.all(
          constatacionDomiciliariaDetails.map(async (detalle: any) => {
        ...
        console.error("Error fetching Constatacion Domiciliaria data:", error);
        setLoading(false);
      }
    }
  ...
  const renderTramitesPorSeccion = (estado: Estado, seccion: string) => {
    ...
    return filteredTramites.map((tramite) => (
      ...
      >
        <p>
          <strong>Tipo de Cliente:</strong> {tramite.tipoCliente || "Desconocido"}
        </p>
        <p>
        ...
        <p>
          <strong>Cliente:</strong> {capitalize(tramite.nombres)} {tramite.apellidoPaterno} {capitalize(tramite.apellidoMaterno) || ""}
        </p>
        <p>
        ...
        {seccion === "Enviar correo/Lost" && (
          <div>
            <p className="text-red-400">
              ...
              {!tramite.documentoCopiaLiteral && "Documento de Copia Literal"}
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <h1 className="text-4xl font-bold mb-8 text-center">
        Dashboard de Constatación Domiciliaria
      </h1>
