...
export default function Home() {
  ...
  // Dentro del useEffect que maneja el fetching de datos:
  useEffect(() => {
  ...
  const handleDelete = async () => {
    ...
    // Encuentra el trámite seleccionado basado en el ID
    const tramite = data?.data.find((item) => item._id === selectedTramite);
    ...
    const type = tramite.tipoTramite; // Asegúrate de que 'tipoTramite' esté disponible en los datos
    const numeroProceso = tramite.numeroProceso; // Usar 'numeroProceso' en lugar de '_id'
  
    if (!type || !numeroProceso) {
      alert(
        "No se encontraron los datos necesarios (tipo de trámite o número de proceso) para el ID seleccionado"
      );
      return;
    }
  ...
  return (
    <div className="text-white bg-gray-900 h-full overflow-auto">
      <h3 className="text-4xl font-bold mb-8 pt-4 text-center">Trámites</h3>
        ...
        <div className="flex flex-col h-[calc(100vh-300px)] overflow-y-auto">
          ...
          <div className="mt-4 flex justify-between items-center">
            <Button
              ...
              onClick={() =>
                setParams((prev) => (handlePreviousPage(), prev))
              }
            ...
            <Button
              className="bg-primary-500 disabled:bg-gray-500"
              onClick={() =>
                setParams((prev) => (handleNextPage(), prev))
              }
